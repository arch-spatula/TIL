...
서버가 응답하고 있는 동안 실행하는 명령입니다. 실행할 명령은 인자로 들어가 있는 콜백화살표함수를 실행합니다.


해쉬는 #입니다. html에서 id값인 url로 접근합니다. #은 fragment identifier라고도 부릅니다.
...
AJAX의 단점은 SEO에 불리합니다. 서버에서 동적으로 가져오기 때문에 크롤러가 보릅니다.

SEO가 중요하면 PJAX를 공부하는 것도 방법입니다.

과거 브라우저 호환문제를 해결하기 위해 polyfill을 사용해야 합니다.
...
var o1 = { name : "kim" };
fn(o1);
console.log(o1);  // 의도하지 않은 사이드이팩트가 발행: 원 { name : "kim" }이 { name: 'lee' }으로 바뀜
```
