...
- [ ] 11장. 메모리와 포인터
  - [ ] 11.2. 메모리 동적 할당 및 관리
    - [x] 11.2.1. 메모리 초기화 및 사용(배열)
    - [x] 11.2.2. 메모리 복사
    - [x] 11.2.3. 메모리 비교(`memcmp( )`, `strcmp( )`)
    - [ ] 11.2.4. 문자열 검색
    - [ ] 11.2.5. 배열 연산자 풀어쓰기
...
- "독하게 시작하는 C"를 읽는데 이제부터 컴퓨터구조와 운영체제를 간접적으로 다루는 영역이 늘어날 것 같습니다.
  - 메모리를 읽기 시작했는데 이제야 틀린 문제가 나왔습니다. 언어를 배우는데 메모리에 대해서 생각하는 것이 틀렸습니다. 이제 학습할 수 있는 부분이 생겼습니다.

### Learned

- vue에서 provide, inject를 사용하는 이유를 다시 생각해볼 수 있었습니다.
  - 컴포넌트 내에서 전역 상태랑 비슷하게 사용하기 위해 사용합니다.
  - store에 넣으면 간섭이 발생합니다. 하지만 provide를 사용하면 컴포넌트 범위로 한정됩니다.
- API 명세서를 어느정도 작성할 수 있게 되었습니다. 물론 초안을 작성하는 것입니다.

### Lacked

- 이번주에는 묘하게 낭비한 시간이 너무 많습니다. 자잘하게 주의가 분산되었습니다.
  - 작은 디버깅과 리팩토링을 계속 시도해볼 수 있는 기회가 있는데 활용을 너무 안 했습니다.
- 자세관리가 필요한 시점이 되었습니다. 자세가 안 좋아 컨디션이 나쁜 것 같습니다.

### Longed(잘하기 위해 필요한 것)

- 수치화된 성과를 찾기 시작해야 합니다. 경력기술서에 작성할 것이 너무 없습니다.
- 옵시디언으로 회사에서 하는 프로젝트를 비공개로 개인적으로 정리해야 합니다. 무슨 작업을 언제 했는지 정리해야 안 잊고 정리할 수 있습니다.
